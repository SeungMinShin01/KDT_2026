...
-- [문제 5]books 테이블에서 장르(genre)가 '소설' 인 도서의 price를기존 가격에서 
-- 2000원 인상하도록 수정하세요.(산술연산자 사용)
...
-- [문제 13]books 테이블에서 price가 15000 이상(>=) 이고 stock이 10 이상(>=) 인 
-- 도서의 title, price, stock을 조회하세요.
SELECT TITLE, PRICE, STOCK FROM BOOKS WHERE PRICE >= 15000 AND STOCK >= 10;

-- [문제 14] books 테이블에서 장르(genre)가 '컴퓨터' 또는 '경제' 인 도서의 title, genre를 조회하세요. 
-- (IN 연산자 사용)
...
-- [문제 19] books 테이블에서 도서명(title)에 '자'가 포함된 도서의 title, author를 조회하세요. 
-- (LIKE '%자%' 사용)
SELECT TITLE, AUTHOR FROM BOOKS WHERE TITLE LIKE '%자%';

-- [문제 20] books 테이블에서 저자(author)가 '김'으로 시작하는 
--3글자인 도서의 title, author를 조회하세요. (LIKE '김__' 사용)
