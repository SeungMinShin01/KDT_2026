...
public class exam1 {
    public static void main(String[] args) {
        ...
        thread2.cacluator = calculator;
        // * 서로 다른 스레드가 동일한 객체 참조
        // 스레드 실행
        thread1.start();
        thread2.start();
    }
...
class 스레드2 extends Thread {
    ...
    @Override
    public void run() {
        cacluator.setMemory(200); // 계산기 메소드에 200 대입
    }
}
...
class 계산기 { // 클래스란? 인스턴스 설계도 (종이 / 실체 아니다. )
    ...
    // [1] 비동기화(기본값) : 200 (두번째 요청이 출력 전 변경함 ) 200 200 출력
    // public void setMemory( int memory ) {}
    // [2] 동기화(synchronized) : 100(두번째요청이 대기상태) 200 출력

    public synchronized void setMemory(int memory) {
        this.memory = memory; // this란? 현재 메소드를 호출한 인스턴스 가리킴

        try {
            Thread.sleep(2000); // 2~3초 대기상태 만들기
