...
public class BoardDao extends BaseDao {
    ...
    public boolean save(BoardDto boardDto) {
        try {
            ...
            // 1.3 기재된 SQL 문법내 ? (와일드카드) 매개변수 값 대입, ps.set타입( ?번호 , 값 );
    ...
    public boolean update(BoardDto boardDto) {
        try {
            ...
            ps.setString(1, boardDto.getContent()); // 1.3 SQL내 ? 매개변수 대입
            ps.setInt(2, boardDto.getNo());
            int result = ps.executeUpdate(); // 1.4 SQL 실행
            if (result == 1) // 1.5 실행결과 반환
                return true;
        } catch (SQLException e) {
            System.out.println(e);
        }
        return false; // 1.5 실행결과 반환
    }
    ...
    public boolean delete(int no) {
        try {
            ...
            ps.setInt(1, no); // SQL 문법내 첫번째 ? 에 매개변수 대입
