...
import java.awt.*; // java내 UI 제공 = 최신
...
public class exam3 {
    public static void main(String[] args) {
        ...
        // [2] 멀티스레드 방법1: 익명구현체, new 인터페이스타입(){}
        // Runnable runnable = new Runnable() { };
        Runnable runnable = new Runnable() {
            // 새로운 스레드가 처리할 메소드 = run( )
            @Override
            public void run() {
                for (int i = 1; i <= 5; i++) {
        ...
        Thread thread1 = new Thread(runnable); // new Thread( 익명 구현체 );
        thread1.start(); // 새로운 스레드가 run() 메소드 호출 = start()
        // [3] 멀티스레드 방법2: 구현체
