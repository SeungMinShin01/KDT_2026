...
if (point >= 90) {
  console.log("A학점");
} // 만약에 point변수가 90 이상이면 A학점
else if (point >= 80) {
  console.log("B학점");
} // 만약에 point변수가 80이상이면 B학점
else {
  console.log("C학점");
} // 그외 C학점
...
// 7. 스프레드 연산자 : ... 배열 또는 개체를 복사할 때 사용, 사용처 : 주소값 변경 목적 (리액트 state / Flutter)
const obj3 = { ...obj1, phone: "010" }; // {...기존객체 , 새로운 속성}
console.log(obj3);
const obj4 = ["010", ...obj2]; // [...기존배열, 새로운 값]
console.log(obj4);
// 8. 구조분해 할당 : 배열 또는 객체에서 값을 분해 해서 각 변수/상수 에 저장
...
printScore(50, printSuccess); // 콜백함수 방식으로 함수 호출, 주의할점: 인수에 함수 전달시 함수실행X 함수정의 O
// 함수명(3 + 3); 인수: 6 // 함수명(plus(3,3) ), 인수:6 // 함수명(plus) , 인수: plus 함수

// 10. 동기식: 먼저 호출한 함수/기능이 결과가 올때까지 대기상태 , 동기화
//     비동기: 먼저 호출한 함수/기능이 결과는 순서 상관없이 결과 반한 상태, axios
// axios 비동기통신이다. 동기화로 만드는 방법, (1) 선언 함수앞에 async , (2) axios 앞에 await
const backLoad = async () => {
    const response = await async axios( );
}
backLoad