...
public class exam4 {
    public static void main(String[] args) {
        ...
        // 1.
        // Integer.parseInt( "문자" ); // 문자 --> 정수 반환 함수
        ...
        // 메소드 래퍼런스 방식 , 주의할점은 메소드 뒤에 ()를 사용하지 않는다. 내부적으로 메소드 호출 하는 구조
        names.stream().map(String::length).forEach(System.out::println);
        // 4. names 리스트내 문자열/이름들을 각각 대입하여 Student 객체를 만드세요
        // 전통방식
 ...
 * // 1.
 * List<MemberDto> list = entityList.stream().map( ( entity ) -> { return
 * MemberDto.from( entity ); } ).toList();
 * // 2.
 * List<MemberDto> list = entityList.stream().map( MemberDto::from ).toList();
 * 
 * return boardRepository.findAll().stream().map( (entity)->{ return
 * BoardDto.from(entity); } ).toList();
