...
// 사용자로부터 새로운 과일 이름을 입력받아, fruitList에 이미 존재하는 과일이면 '이미 존재하는 과일입니다.'를 출력
//  존재하지 않는 과일이면 배열에 추가한 뒤, 변경된 배열 전체를 출력하는 프로그램을 작성하시오.

/*
    5. 배열 요소 찾기 day03 - exam1.js
    .indexof(찾을값)
...
/*
    charAt() 함수 – 인수로 전달된 문자열의 인덱스 위치에 해당하는 문자를 반환하는 함수
        - charAt() 함수는 문자열에서 원하는 위치의 문자를 쉽게 추출할 수 있습니다.
    ...
    참고자료: https://codingeverybody.kr/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8-charat-%ED%95%A8%EC%88%98/
...
/*
    배열과 문자열의 문자열을 이용한 인덱스 접근

    문자열이나 배열의 인덱스는 숫자로도 접근할 수 있지만, 
    숫자로 형변환 가능한 문자열으로도 접근할 수 있다.

    let alphabetString = "abcdefghij";
 
    console.log(alphabetString);
    console.log(alphabetString[2]);
    console.log(alphabetString['2']);
    ...
    c
    c
    참고자료: https://20002100.tistory.com/entry/%EB%B0%B0%EC%97%B4%EA%B3%BC-%EB%AC%B8%EC%9E%90%EC%97%B4%EC%9D%98-%EB%AC%B8%EC%9E%90%EC%97%B4%EC%9D%84-%EC%9D%B4%EC%9A%A9%ED%95%9C-%EC%9D%B8%EB%8D%B1%EC%8A%A4-%EC%A0%91%EA%B7%BC-ex-arr0
*/
...
// 사용자로부터 차량 번호를 입력받아,
// carArray에서 해당 차량을 찾은 뒤 locationArray에 있는 그 차량의 주차 위치를 찾아 출력하는 프로그램을 작성하시오. 만약 해당하는 차량 번호가 없다면 '차량이 존재하지 않습니다.'를 출력하시오.
...
let wishList = prompt("제외하고 싶은 과목명 입력");
let courseList = ["수학", "영어", "과학", "국어"];
let listIndex;
if (courseList.includes(wishList) == true) {
  listIndex = courseList.indexOf(wishList);
  courseList.splice(listIndex, 1);
  console.log(courseList);
} else console.log("해당 과목은 수강 신청 목록에 없습니다.");
