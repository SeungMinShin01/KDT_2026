...
let counter = 0;
function increaseCount(number3) {
...
printNumbers(10);
/*예제 6: prompt와 함수 활용
prompt를 이용해 사용자로부터 이름을 입력받으시오. 입력받은 이름을 매개변수로 받아, 해당 이름이 저장된 userList 전역 배열에 push하는 addUser 함수를 정의하고 
호출하시오.*/
...
function isPositive(number4) {
  if (number4 > 0) result = true;
  else result = false;
  return result;
}
/*예제 9: 전역 변수와 지역 변수
